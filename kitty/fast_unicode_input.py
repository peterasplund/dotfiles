...
favorites_path = os.path.join(config_dir, 'fast-unicode-input-favorites.conf')
INDEX_CHAR = '.'
INDEX_BASE = 36
...
def name(cp: Union[int, str]) -> str:
    from kittens.unicode_input.unicode_names import name_for_codepoint
...
class UnicodeInput(Handler):
    ...
    def __init__(self, cached_values: Dict[str, Any], emoji_variation: str = 'none') -> None:
        ...
        self.choice_line = ''
        # fast_unicode_input: Always start in HEX
    ...
    def on_text(self, text: str, in_bracketed_paste: bool = False) -> None:
        ...
        # fast_unicode_input: This is the modification which makes it end on
        # space in unicode input
        if self.mode is HEX and text[-1] == ' ':
            # Required to update which character to return
            self.update_current_char()
            self.quit_loop(0)
            return
...
def main(args: List[str]) -> Optional[str]:
    ...
    loop = Loop()
    with cached_values_for('fast-unicode-input') as cached_values:
